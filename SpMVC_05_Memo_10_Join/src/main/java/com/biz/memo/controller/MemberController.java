 ...
 * 		spring에서는 SessionAttribute일 경우 Gabage Collection이 적용
 * 
 ...
 * 		서버의 메모리공간을 많이 차지하여
 * 		서버 성능에 문제를 일으키기도 한다.
 *		GC(Gabage Collection)을 적용할 수 없다.
 * 		표준 HTTP 프로토콜에 정의된 규칙을 사용한다.
 */
...
public class MemberController {
	...
	public String login(UserDTO userDTO, Model model, HttpSession httpSession) {
		...
		if(userDTO.getU_id().equalsIgnoreCase(getUserDTO.getU_id()) &&
				userDTO.getU_password().equalsIgnoreCase(getUserDTO.getU_password())) {
			
			httpSession.setMaxInactiveInterval(10*60); // tl
