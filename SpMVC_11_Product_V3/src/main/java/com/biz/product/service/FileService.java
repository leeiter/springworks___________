...
public class FileService {
	 ...
	 * List에 추가한 후 파일 이름들 List를 Controller로 return
	 */
	public List<ProFileDTO> filesUp(MultipartHttpServletRequest u_files) {
		
		List<ProFileDTO> upFileNames = new ArrayList<ProFileDTO>();
		
		/*
		 * 만약 개별파일을 업로드 하는 중에 문제가 발생을 하면
		 * controller에게 null 값을 return 하여 파일 업로드에 
		 * 문제가 발생했음을 알리자
		 */
		try {
	...
	public String fileUP(MultipartFile u_file) throws Exception {
		...
		String upLoadPath = winFilePath; // upLoadPath : /product/files/
		...
		if(u_file != null) {
			 ...
			 * original 파일이름을 사용하면
			 * 해킹하고를 일의킬수 있기 때문에
			 파일이름ㅇ르 UUID로 설정하여 DB table에 저장하자
			 */
