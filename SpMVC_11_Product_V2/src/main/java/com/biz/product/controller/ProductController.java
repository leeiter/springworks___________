...
public class ProductController {
	...
	public String input(@ModelAttribute ProductDTO proDTO, @RequestParam("u_file") MultipartFile u_file) {
		...
		// 파일을 선택하지 않았으면
		// 파일에 관한 처리를 수행하지 말자
		// 업로드할 파일ㅇ르 선택했을 때만 파일에 관한 처리를 하자
		if(!u_file.isEmpty()) {
			
			/*
			 * update를 수행할 때
			 * 이미 업로드 된 파일이 있으면 기존이 파일을 삭제하고
			 * 새로운 파일을 업로드 해야 하므로
			...
			/*
			 * web에서 파일이 전송되어 오면 fService.fileUp() method에게 파일을 어딘 가에 저장해 달라 라고 요청
			 * 
			 * 
			 */
	...
	*/
	...
	// @ResponseBody를 사용할 떄는
	// 꼭 produces를 설정하는 것이 좋다.
	// 특히 한글 데이터를 response할 떄는 반드시 !!!
	...
	// 절대 사용자 만든 vo, dto에는 적용하면 안된다.
	
	// 혹시 값을 보내지 않아도 오류를 내지 마라
	required = false,
	// 값을 보내지 않으면 없음이라는 문자열을 세팅하라
	defaultValue = "없음")
