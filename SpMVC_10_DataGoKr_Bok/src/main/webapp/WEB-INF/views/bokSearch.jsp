...
<script type="text/javascript">
	$(function() {
		...
		$("#btn-search").click(function() {
			...
			// 서버에 전송할 수 있도록 (문자열 화) 하여라
			var queryString = $("form").serialize();
			
			
			/*
			Astnc JavaScript XML
			비동기방식으로 서버에 request를 보낼 때 사용하는
			Js의 내장 프로토콜
			1. web에서 ajax에게 req를 요청하면
			2. ajax는 ok를 보내서 web이 다른 작업ㅇ르 수행할 수 있도록 하고
			3. ajax는 매개변수로 설정된 항목에 의해서 서번에 request를 보낸다.
			4. 서버넹서 정상적으로 response를 보내면
			...
			*/
			$.ajax({
				...
				success : function(result) {		// 결과가 성공했을 떄
					// resFunc(result)
