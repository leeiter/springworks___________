 ...
 * 
 * HTTP 프로토콜의 성질
 * web form에 입력된 data ===> server 전송되고 나면
 * 그 데이터는 공중으로 사라진다.
 * server ==> web browser에 어떤 결과를 보내고 나면
 * 마찬가지로 데이터는 사라지고, 연결도 모두 종료된다.
 * 
 * 하지만
 * web == server 간에 어떤 데이터를 일정하게 유지하고 싶을 때가 있다
 * (login 등)
 * 이럴 때는 session이라는 공간에 데이터를 저장해 두면
...
//				.build();
	...
	public String search(@ModelAttribute("bokSearchDTO") BokSearchDTO bokSearchDTO, Model model, String strDumy) throws UnsupportedEncodingException {
	...
	public String searchAPI(@ModelAttribute("bokSearchDTO") BokSearchDTO bokSearchDTO, Model model, String strDumy) throws UnsupportedEncodingException {
	...
	public String detail(@RequestParam("id") String servId, Model model) throws UnsupportedEncodingException {
